...
function getPhase(t) { return PHASES[t.status] || PHASES.open; }
function getOpenPct(t) { return getPhase(t).pct; }

function TpProgress({ trade }) {
  const tps = [
    { key: "tp1", hit: trade.tp1_hit, label: "TP1" },
    { key: "tp2", hit: trade.tp2_hit, label: "TP2" },
    { key: "tp3", hit: trade.tp3_hit, label: "TP3" },
  ];
  return (
...
function ActiveTradeCard({ trade }) {
  ...
  return (
    <div style={{
      background: "#0a0a0a", border: `1px solid ${phase.color}22`,
      borderLeft: `3px solid ${phase.color}`, borderRadius: 8,
      padding: "16px 20px", marginBottom: 12,
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, color: isLong ? "#00ff88" : "#ff4455" }}>{isLong ? "▲ LONG" : "▼ SHORT"}</span>
          <span style={{ fontSize: 11, color: "#555" }}>{trade.symbol}</span>
          <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 3, background: "#1a1a2a", color: "#7788ff", letterSpacing: 1, textTransform: "uppercase" }}>{trade.setup_type}</span>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 10, padding: "3px 10px", borderRadius: 4, background: phase.bg, color: phase.color, letterSpacing: 1, fontWeight: 700 }}>{phase.label}</span>
      ...
      {trade.reason && <div style={{ marginTop: 8, fontSize: 10, color: "#333", fontStyle: "italic" }}>{trade.reason}</div>}
    </div>
  );
}
...
function StatCard({ label, value, sub, accent }) {
  return (
    <div style={{ background: "#0d0d0d", border: `1px solid ${accent ? accent + "33" : "#1a1a1a"}`, borderRadius: 8, padding: "16px 20px", minWidth: 130, flex: 1 }}>
...
export default function Dashboard() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    } catch { setError("Kan bot API niet bereiken — draait hij?"); }
  }, []);
  ...
  async function startBot() {
    setLoading(true);
    try {
      await fetch(`${API_URL}/start`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...config, timeframe: "15m" }) });
      await fetchStatus();
  ...
  async function stopBot() {
    setLoading(true);
    try { await fetch(`${API_URL}/stop`, { method: "POST" }); await fetchStatus(); }
  ...
  const winRate = closedTrades.length > 0 ? Math.round((status.winning_trades / closedTrades.length) * 100) : null;

  return (
    <div style={{ minHeight: "100vh", background: "#060606", color: "#e0e0e0", fontFamily: "'JetBrains Mono', monospace", padding: "28px 36px" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;700&family=Syne:wght@700;800&display=swap');
        * { box-sizing: border-box; }
        ::-webkit-scrollbar { width: 4px; background: #0a0a0a; }
        ::-webkit-scrollbar-thumb { background: #222; border-radius: 2px; }
        .btn-primary { background: #00ff88; color: #000; border: none; border-radius: 6px; padding: 11px 24px; font-family: inherit; font-size: 12px; font-weight: 700; letter-spacing: 1.5px; cursor: pointer; text-transform: uppercase; transition: opacity 0.15s; width: 100%; }
        .btn-primary:hover { opacity: 0.85; }
        .btn-primary:disabled { opacity: 0.3; cursor: not-allowed; }
        .btn-danger { background: transparent; color: #ff4455; border: 1px solid #ff445544; border-radius: 6px; padding: 11px 24px; font-family: inherit; font-size: 12px; font-weight: 700; letter-spacing: 1.5px; cursor: pointer; text-transform: uppercase; transition: background 0.15s; width: 100%; }
        .btn-danger:hover { background: #ff445511; }
        select, input { background: #0d0d0d; border: 1px solid #1e1e1e; color: #e0e0e0; font-family: inherit; font-size: 12px; padding: 8px 12px; border-radius: 6px; outline: none; width: 100%; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, letterSpacing: -0.5 }}>₿ TRADE<span style={{ color: "#00ff88" }}>BOT</span></div>
          <div style={{ color: "#333", fontSize: 10, letterSpacing: 2.5, marginTop: 2 }}>OKX · DOOPIECASH METHOD · 15M/1H</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div className="pulse" style={{ width: 7, height: 7, borderRadius: "50%", background: isRunning ? "#00ff88" : "#2a2a2a" }} />
          <span style={{ fontSize: 10, color: isRunning ? "#00ff88" : "#444", letterSpacing: 2 }}>{isRunning ? "LIVE" : "OFFLINE"}</span>
        </div>
      </div>

      {error && <div style={{ background: "#180808", border: "1px solid #ff445533", borderRadius: 6, padding: "9px 14px", marginBottom: 20, color: "#ff4455", fontSize: 11 }}>⚠ {error}</div>}
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
        ...
        <StatCard label="Last Signal" value={status?.last_setup?.toUpperCase() || "—"} sub={status?.last_signal?.toUpperCase() || ""} accent={status?.last_signal === "buy" ? "#00ff88" : status?.last_signal === "sell" ? "#ff4455" : undefined} />
      </div>
        ...
        <div>
          <div style={{ background: "#0a0a0a", border: "1px solid #161616", borderRadius: 10, padding: 20, marginBottom: 16 }}>
            ...
            <label style={{ display: "block", marginBottom: 20 }}>
              ...
              <input type="number" min="0.001" max="0.05" step="0.001" value={config.risk_per_trade} onChange={e => setConfig({ ...config, risk_per_trade: parseFloat(e.target.value) })} disabled={isRunning} />
              <div style={{ fontSize: 10, color: "#2a2a2a", marginTop: 4 }}>{(config.risk_per_trade * 100).toFixed(1)}% per trade</div>
            </label>
            {!isRunning
              ? <button className="btn-primary" onClick={startBot} disabled={loading}>{loading ? "Starten..." : "▶️ Start Bot"}</button>
        ...
        <div>
          <div style={{ marginBottom: 20 }}>
            ...
            {activeTrades.length === 0
              ? <div style={{ background: "#0a0a0a", border: "1px solid #161616", borderRadius: 8, padding: "32px 20px", textAlign: "center", color: "#2a2a2a", fontSize: 12 }}>Geen actieve trades.<br /><span style={{ fontSize: 10, color: "#1a1a1a" }}>Bot zoekt naar een setup...</span></div>
              : activeTrades.map((t, i) => <ActiveTradeCard key={t.id || i} trade={t} />)
            }
          </div>
          <div>
            <div style={{ fontSize: 10, letterSpacing: 2, color: "#444", marginBottom: 14, textTransform: "uppercase" }}>Gesloten trades ({closedTrades.length})</div>
                          ...
                          <td style={{ padding: "8px 12px" }}>
                            <div style={{ display: "flex", gap: 3 }}>
                              {["tp1_hit","tp2_hit","tp3_hit"].map((k, idx) => (
                                <span key={k} style={{ fontSize: 9, padding: "1px 5px", borderRadius: 2, background: t[k] ? "#001a0d" : "#111", color: t[k] ? "#00ff88" : "#2a2a2a" }}>TP{idx+1}</span>
                              ))}
                            </div>
                          </td>
